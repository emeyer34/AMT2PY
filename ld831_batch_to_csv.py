...
INPUT_PATH = r"C:\Users\Emeyer\OneDrive - DOI\Desktop\DesktopTemp\NSNSD\Projects\CS_Py\TEST\CANYCOLO_2025"
OUTPUT_PATH = r"C:\Users\Emeyer\OneDrive - DOI\Desktop\DesktopTemp\NSNSD\Projects\CS_Py\TEST\CANYCOLO_2025\NVSPL_python\Max_gust_NVSPL"
CREATE_SITE_FOLDERS = True   # per-site subfolders
RECURSIVE = True             # recurse subfolders for .831
...
LD_TZ  = "America/Denver"
MET_TZ = "America/Denver"
...
def _maybe_localize_unix_seconds(sec: int, ld_tz: Optional[str]) -> dt.datetime:
    ts = UNIX_EPOCH + dt.timedelta(seconds=sec)  # UTC aware
    if ld_tz:
        tz = _tz(ld_tz)
        ts = ts.astimezone(tz)
    return ts.replace(tzinfo=None)  # NVSPL uses naive strings
...
def _extract_dt_string(s: str) -> Optional[str]:
    """
    Extracts a date-time substring from a messy cell using regex.
    Supports:
      - US style: MM/DD/(YY|YYYY) HH:MM(:SS)? with optional AM/PM
      - ISO style: YYYY-MM-DD[ T]HH:MM(:SS)?(.frac)? with optional 'Z' or ±HH:MM
    """
    if not s:
        return None
    ...
    # US m/d/yy or m/d/yyyy with optional seconds + AM/PM
    m = re.search(
    ...
    # ISO: YYYY-MM-DD[ T]HH:MM(:SS)?(.frac)? with optional trailing Z or ±HH:MM
    m = re.search(
...
def _parse_dt_flex(s: str, fmt: Optional[str]) -> Optional[dt.datetime]:
    ...
    Flexible datetime parser:
      - If fmt is provided, tries that directly.
      - Otherwise, extracts a dt substring and attempts multiple formats (24h + AM/PM + ISO),
        including two-digit year (%y).
    """
    s = (s or "").strip()
    if not s:
        return None
    ...
    # US formats with 4-digit and 2-digit year, 24h + AM/PM
...
def _align_naive(ts: dt.datetime, src_tz: Optional[str], dst_tz: Optional[str]) -> dt.datetime:
    if src_tz and dst_tz:
        ...
        ts = ts.replace(tzinfo=tz_src).astimezone(tz_dst)
    elif src_tz and not dst_tz:
        tz_src = _tz(src_tz)
        ts = ts.replace(tzinfo=tz_src).astimezone(dt.timezone.utc).replace(tzinfo=None)
        return ts
...
def _try_load(csv_path: Path,
    ...
    with csv_path.open("r", encoding=encoding, newline="") as f:
        ...
        for row in reader:
            ...
            # Skip Plot Title line (single field with title)
            if len(row) == 1 and "Plot Title" in row[0]:
                stats["header_skips"] += 1
                continue

            # Skip "indexed" header row
            ...
            # Skip header-like tokens in time column (Date/Time words)
